...
test_resume_instance = resume_df.iloc[200,1] #change first number to change resume instance (e.g [10,1] for the resume in index 10)
...
def test_all_scoring_functions(test_resume=test_resume_instance, job_df=job_df, save_csv=True):
    ...
    if save_csv:
        top10_df.to_csv(f'test_results/aggregate_test_results_resumeid={test_resume_id}.csv')

    print(f'finished tests for {test_resume_id}')
    return top10_df
